...
ENV_MOBILE_PASSWORD = 'MOBILE_PASSWORD'  # 多个账号密码
...
def print(text, *args, **kw):
    with threading.Lock():
        original_print(text, *args, **kw)

# 导入通知模块（假设notify.py与当前脚本在同一目录）
try:
    from notify import send as send_notification
    print("通知模块加载成功")
except ImportError:
    print("⚠️ 未找到notify.py，通知功能将不可用")
...
if __name__ == "__main__":
    ...
    if single_notification == "true":
        ...
        send_notification(combined_title, combined_content)  # 使用notify模块发送通知
